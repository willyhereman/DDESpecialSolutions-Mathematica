...
*  : Last updated : Sunday, January 25, 2009 at 01:30 AM by Hereman
...
"The solution(s) `1` do not fix all the values of \!\(M\_i\).  We will now "<>
"generate all the possible values for the free \!\(M\_i\) up to the "<>
"user-supplied option MaxDegreeOfThePolynomial which is set to 3 "<>
...
DDESpecialSolutions[eqns_List, funcs_List, dvars_List, cvars_List,
  ...
  (
    ...
    If[VerboseTest,
      Print["The given system of differential-difference equations: "];
      Print[ eqns /.
    ...
    If[VerboseTest,
      Print["Transform the system of differential-difference equations "<>
            "into nonlinear DDEs in T = Tanh."]
    ];
    ...
    If[VerboseTest,
      Print["Determine the maximal degree of the polynomial solutions in T."]
    ];
    ...
    If[VerboseTest,
      If[( (SymbolicTest /. {opts} /. Options[DDESpecialSolutions]) ||
        ...
        Print["Build and test the travelling wave solutions."], 
        (* else *)
...
mSolve`ListFormation[eqnList0_, myTrackingVariableMax_]:=
  Block[{ mPowerListFormationDebug = False,
    ...
    If[mPowerListFormationDebug,
      Print["The system after + is replaced by {} via + -> {} is: "];
      Print[CleanUpFunction[eqnList]];
    ];
...
(* WH:01/23/2009 switch buildSystemDebug flag to True for testing, *)
(* set back to False afterwards *)
DDESpecialSolutionsBuildSystem[mSoln_List, system_List, dvars_List, 
  ...
  Block[{buildSystemDebug = False, (* Debug Boolean. *)
    ...
    If[Verbose /. {opts} /. Options[DDESpecialSolutions],
      Print["Seek polynomial solutions in T of the form:"];
...
(* WH:01/23/2009 added pureuRules to see the format of such rules *)
    ...
    If[buildSystemDebug,
      Print["Getting rid of non-zeros, one gets: "];
      Print[CleanUpFunction[ newSystem ]];
    ];
...
DDESpecialSolutionsBuildSolutions[solution_, mSoln_, 
   ...
   Block[{buildSolnDebug = False, (* Debug Boolean *)
    ...
    If[NumericTest /. {opts} /. Options[DDESpecialSolutions],
      ...
      If[buildSolnDebug,
        Print["The numerically tested solutions: "];
    ...
    If[SymbolicTest /. {opts} /. Options[DDESpecialSolutions],
      ...
      If[(Map[Union, symbolicTestSolutions, 3] //. {{}}->{}) != {},
        ...
        CellPrint[
          Cell[
            "These results did not easily simplify to zero. " <>
            "Please test these solutions by hand or interactively with " <>
            "Mathematica. ",
            "Message"
          ]
...
Print["Version 2 was last updated: Friday, January 23, 2009 by Hereman."];
